...
@else
    <div class="group relative block p-6 bg-surface rounded-lg border border-border hover:border-border-light transition-colors">
        <a
            href="{{ route('project.overview', $project) }}"
            wire:navigate
            class="block"
        >
            <h3 class="font-mono text-sm font-medium text-text-pri truncate pr-6">{{ $project->name }}</h3>
